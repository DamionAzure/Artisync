...
function renderProceduralFeed() {
    ...
    feedData.forEach(post => {
        ...
        // 1. Relational Logic: Calculate Span
        let spanClass = 'span-p';
        if (post.type === 'casting') spanClass = 'span-cc';
        else if (post.type === 'resource') spanClass = 'span-r';
        else if (post.cat === userProfile.primaryField) spanClass = 'span-prominent-p';

        item.className = `masonry-item group ${spanClass}`;

        // 2. Build the DOM with the metadata span
        card.innerHTML = `
